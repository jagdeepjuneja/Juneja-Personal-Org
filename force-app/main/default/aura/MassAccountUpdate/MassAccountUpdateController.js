({
    ...
    activateAccounts: function(component, event, helper) {
        ...
        var inactiveAccMap = component.get("v.inactiveAccMap");
        var activateAccount = true;
        
        if(!Array.isArray(checkvalue)){
            if (checkvalue.get("v.value") == true) {
                Object.keys(inactiveAccMap).forEach(function(key) {
                    if(inactiveAccMap[key].key === checkvalue.get("v.text")){
                        //alert('You can not update '+theMap[key].value);
                        //alert('first');
                        helper.showToast(component, event, "error", "Error!", 'You can not update '+ inactiveAccMap[key].value);
            ...
            for (var i = 0; i < checkvalue.length; i++) {
                if (checkvalue[i].get("v.value") == true) {
                    Object.keys(inactiveAccMap).forEach(function(key) {
                        if(inactiveAccMap[key].key === checkvalue[i].get("v.text")){
                            //alert('You can not update '+theMap[key].value);
                            //alert('second');
                            helper.showToast(component, event, "error", "Error!", 'You can not update '+ inactiveAccMap[key].value);
                            activateAccount = false;
