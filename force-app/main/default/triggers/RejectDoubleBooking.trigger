trigger RejectDoubleBooking on Session_Speaker__c (before insert, before update) {
    ...
    for(Session_Speaker__c ss : [SELECT Speaker__c , Session__c, Session__r.Session_Date__c 
                                  FROM Session_Speaker__c
                                  WHERE NOT ID IN: trigger.new]){
      uniqueSet.add(ss.Speaker__c + '__' + ss.Session__r.Session_Date__c );                              
    }
    System.debug('test uniqueSet is '+uniqueSet);
    
    for(Session_Speaker__c ss : [SELECT Speaker__c , Session__c, Session__r.Session_Date__c 
                                  FROM Session_Speaker__c
                                  WHERE ID IN: trigger.new]){
      if(!uniqueSet.contains(ss.Speaker__c + '__' + ss.Session__r.Session_Date__c )){
        System.debug('test in if block');
        uniqueSet.add(ss.Speaker__c + '__' + ss.Session__r.Session_Date__c );
      }
      else{
        ss.addError('Speaker is already booked at this time');
      }
    }*/
